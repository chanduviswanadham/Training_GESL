...
long myIoctl(struct file *filep1, unsigned int pid, unsigned long p)
{
	...
	printk(KERN_INFO "*****************my proc/%d/maps***************\n", pid);
...
//	printk (KERN_INFO "mmaps:%p\n", task -> mm -> mmap);
	...
	//	printk (KERN_INFO "values of vm_end is : %lx\n", v_add -> vm_end);
		printk (KERN_INFO "%lx\t%lx\t", v_add -> vm_start, v_add -> vm_end);
		v_add = ( struct vm_are_struct * ) v_add -> vm_file;
//	printk (KERN_INFO "flags:%lx\t%lx\n", v_add -> vm_flags,**v_add);
//		printk("\n*********************************************\n");
		v_add = v_add -> vm_next;
	} while(v_add != NULL);
