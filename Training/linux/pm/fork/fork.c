...
int main()
{
	...
	*heap = 50;		//heap data
	...
	if(pid > 0) {	//parent process
		...
		printf("a value after fork in parent:%d\t%p\n", a, &a);
		printf("b value after fork in parent:%d\t%p\n", b, &b);
		printf("heap value after fork in parent:%d\t%p\n", *heap, &heap);
		...
		*heap = 55;	//modifing the heap data
		...
		write(sd, "Software Limited", 15);		//writing string into the file
		close(sd);

		//printing the values of different sections

		printf("a value after fork in child:%d\t%p\n", a, &a);
		printf("b value after fork in child:%d\t%p\n", b, &b);
		printf("heap value after fork in child:%d\t%p\n", *heap, &heap);
